...
type Props = { images: Image[]; formId?: string };

export default function ServiceGallery({ images, formId = 'quote' }: Props) {
  ...
  return (
    <>
      <div className="gal" id={formId === 'quote' ? 'gallery' : 'gallery'}>
        {images.map((img, i) => (
          <figure
            key={img.src + i}
            data-cap={img.cap}
      ...
      <div className="cta-row cta-pair">
        <a
          ...
          href="#quote"
          onClick={() => trackConversion('cta_click', { location: 'service_gallery', type: 'quote' })}
        >
          GET A FREE QUOTE
        </a>
